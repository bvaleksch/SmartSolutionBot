...
from smart_solution.bot.routers.admin_leaderboard import router as AdminLeaderboardRouter
...
def setup_routers(dp: Dispatcher) -> None:
    ...
    dp.include_router(AdminLeaderboardRouter)
...
async def main() -> None:
    ...
    db = DataBase()
    session_maker = async_sessionmaker(bind=db.engine, expire_on_commit=False, autoflush=False)
    storage = SQLAlchemyStorage(sessionmaker=session_maker, metadata=MetaData())
